                ...
                if isinstance(data, list):
                    for agent_idx, item in enumerate(data, start=1):
                        if isinstance(item, dict):
                            ...
                            entropy_mean = item.get("entropy")
                            Score = item.get("temperature")
...
if __name__ == "__main__":
    ...
    base_path_single = "./Results/Single_Result"
    base_path_multi = "./Results/Multidis_Result"
    base_path_multiteacher='./Results/Multiteacher_Result'
    base_path_multireview = "./Results/Multireview_Result"
    #base_path_multi_decode='D:\LLM-Discussion\Results\\temp'
    data_single = load_data_multi(base_path_single)
    data_multi=load_data_multi(base_path_multi)
    data_multiteacher=load_data_multi(base_path_multiteacher)
    data_multireview=load_data_multi(base_path_multireview)
    #data_multi_discussion = load_data_multi_decode(base_path_multi_decode_discussion)
    #data_multi_discussion = load_data_multi(base_path_multi_discussion)
    reference_embedding = np.load("./Evaluation/Rule_based/all_embedding.npy")
    cosine_similarity_dict_single = calculate_similarity_for_all_stories(data_single, reference_embedding)
    cosine_similarity_dict_multi = calculate_similarity_for_all_stories(data_multi, reference_embedding)
    cosine_similarity_dict_multiteacher = calculate_similarity_for_all_stories(data_multiteacher, reference_embedding)
    cosine_similarity_dict_multireview = calculate_similarity_for_all_stories(data_multireview, reference_embedding)
    with open("./Evaluation/Rule_based/q_dist.json", "r") as json_file:
        q_dist = json.load(json_file)
    kl_divergence_dict_single = calculate_kl_divergence_for_all_stories(data_single, q_dist)
    kl_divergence_dict_multi = calculate_kl_divergence_for_all_stories(data_multi, q_dist)
    kl_divergence_dict_multiteacher = calculate_kl_divergence_for_all_stories(data_multiteacher, q_dist)
    kl_divergence_dict_multireview = calculate_kl_divergence_for_all_stories(data_multireview, q_dist)
    filtered_result_single = filter_stories(kl_divergence_dict_single)
    filtered_result_multi = filter_stories(kl_divergence_dict_multi)
    filtered_result_multiteacher = filter_stories(kl_divergence_dict_multiteacher)
    filtered_result_multireview = filter_stories(kl_divergence_dict_multireview)
    merged_result = merge_data([
     ("single", filtered_result_single),
    ("multidis", filtered_result_multi),
    ("multiteacher",filtered_result_multiteacher),
    ("multiteacher",filtered_result_multiteacher)])
    #merged_result = merge_data([("multi", data_multi_discussion)])
    standard_result = standardize_values(merged_result)
    file_name = "./Evaluation/Rule_based/Statistics.json"
